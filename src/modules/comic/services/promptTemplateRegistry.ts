...
export const VARIABLE_REGISTRY: Partial<Record<TemplateType, PromptVariableSpec[]>> = {
  ...
  'panel-prompt-chapter': [
    ...
    {
      name: '全章资产设定',
      desc: '本章各分镜绑定资产的视觉设定，按「分镜N」逐镜分组，替换逐镜模式的「绑定资产」',
      emptyText: '（本章分镜没有绑定资产）',
    },
    {
      name: '全章参考图清单',
...
画面描述中必须用给定的图号引用参考图（如「使用图3保持萧薰儿与第1格一致」）；图号一律以【本镜资产参考图清单】为准，不要自己数图、不要改写或编造图号。
不要写全局画风、共用属性（前置/后置共用属性）的内容 —— 它们由系统在生成后按前/后自动拼接到这条描述之外，写进描述里会重复。`,
  'panel-prompt-chapter': `只输出中文 Markdown，不要解释、代码块或 JSON。
为每一镜各输出一段画面描述，逐镜之间用【分镜N】单独一行分段（N 必须与给定分镜的序号完全一致，按序号递增，不遗漏、不新增、不打乱顺序）。
每段内部是一段完整、连贯的中文描述，不要分点、不要对白或旁白。
画面描述中必须用给定的图号引用参考图（如「使用图3保持萧薰儿与第1格一致」）；图号一律以该镜自己的【资产参考图清单】为准，不要自己数图、不要改写或编造图号。
不要写全局画风、共用属性（前置/后置共用属性）的内容 —— 它们由系统在生成后按前/后自动拼接到各镜描述之外，写进描述里会重复。`,
}
...
- 不得编造与清单信息冲突的细节，不要出现镜头语言（如特写、仰视等）。`,
  ...
  'panel-prompt': {
    name: '分镜画面描述',
    description: '按分镜内容、资产视觉状态与资产参考图号清单，逐镜生成可直接生图的画面描述。',
...
【写作要求】
- 描述本镜画面：人物位置与动作、表情、场景环境、光线氛围、构图景别，按本镜的镜头类型组织画面；
- 出场资产的外观、服饰、发型、配色等固定特征必须与上面的资产视觉设定完全一致，不得改写或省略；
- 必须按【本镜资产参考图清单】用图号声明参考图的用途，例如「使用图3保持萧薰儿与第1格一致」；图号只能引用清单里有的，不要自己数图、不要改写或编造图号；
- 与前文分镜保持画面连续：人物位置关系、服装状态、光线、场景细节；
- 严格基于给定信息撰写，不要补写原文没有的细节；不写对白与旁白，只描述画面本身；
- 不写全局画风与共用属性内容（由系统在生成后自动拼接到本描述的前后）。`,
  },
  'panel-prompt-chapter': {
    name: '分镜画面描述（整章一次生成）',
    description: '一次把本章全部分镜交给模型，按【分镜N】分段输出各镜画面描述。',
    content: `你是一名专业的漫画分镜画面描述师。下面给出本章的**全部分镜**、**各镜绑定的资产视觉设定**与**各镜的资产参考图号清单**，请为每一镜各写出一段可直接用于漫画生图的画面描述。
...
【写作要求】
- 逐镜输出：每一镜各一段画面描述，用【分镜N】单独一行开头，N 必须与上面给定的分镜序号完全一致，按序号递增，不遗漏、不新增、不打乱顺序；
- 每段描述该镜的人物位置与动作、表情、场景环境、光线氛围、构图景别，按该镜的镜头类型组织画面；
- 出场资产的外观、服饰、发型、配色等固定特征必须与该镜的资产视觉设定完全一致，不得改写或省略；
- 必须按该镜自己的【资产参考图清单】用图号声明参考图的用途，例如「使用图3保持萧薰儿与第1格一致」；图号只能引用该镜清单里有的，不要自己数图、不要改写或编造图号；
- 各镜之间保持画面连续：人物位置关系、服装状态、光线、场景细节；
- 严格基于给定信息撰写，不要补写原文没有的细节；不写对白与旁白，只描述画面本身；
- 不写全局画风与共用属性内容（由系统在生成后自动拼接到各镜描述的前后）。`,
  },
}
