    ...
    <Teleport to="body">
      <Transition name="fade">
        <div
          ...
          @click.self="closeAiRewriteModal"
        >
          <div
            class="w-[900px] h-[85vh] flex flex-col rounded-xl bg-surface dark:bg-slate-800 border border-border-subtle shadow-2xl shadow-black/40 overflow-hidden"
            @click.stop
          >
            <!-- 弹窗头部 -->
            <div
            ...
            >
              <div class="flex items-center gap-2">
                ...
                <h2 class="text-base font-semibold text-text-primary">AI 改写</h2>
                <span class="text-xs text-text-muted">
                  {{ viewMode === "prompt" ? "提示词模式" : "JSON 模式" }}
                </span>
              </div>
              <button
            ...
            <div class="flex-1 flex min-h-0">
              <!-- 左侧：输入区域 -->
              <div
                class="w-[380px] flex flex-col border-r border-border-subtle shrink-0"
              >
                <!-- 配置区域 -->
                <div
                  class="shrink-0 px-5 py-3 border-b border-border-subtle space-y-2.5"
                >
                  <label class="text-[11px] text-text-secondary block"
                    >选择模型</label
                  >
                  <div class="flex items-center gap-3">
                    <select
                      v-model="selectedLlmModelId"
                      class="flex-1 bg-input-bg border border-border-subtle rounded-lg px-3 py-1.5 text-sm text-text-primary focus:outline-none focus:border-purple-500/30"
                    >
                    ...
                    >
                      <input
                        v-model="includeExistingContent"
                        type="checkbox"
                        class="w-3.5 h-3.5 rounded border-border-subtle bg-input-bg text-purple-500 focus:ring-purple-500/30 focus:ring-offset-0"
                      />
                ...
                <div class="flex-1 p-5">
                  <textarea
                    v-model="aiPrompt"
                    class="w-full h-full bg-input-bg border border-border-subtle rounded-lg px-4 py-3 text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-purple-500/30 resize-none"
                    placeholder="输入改写要求，如：&#10;- 让对话更生动有趣&#10;- 增加更多细节描写&#10;- 调整语气更正式/轻松&#10;- 简化内容..."
                  ></textarea>
                </div>

                <!-- 发送按钮 -->
                <div class="shrink-0 px-5 py-3 border-t border-border-subtle">
                  <button
                    class="w-full px-4 py-2.5 bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-lg hover:opacity-90 text-sm font-medium flex items-center justify-center gap-2 transition-opacity shadow-lg shadow-purple-500/20 disabled:opacity-50 disabled:pointer-events-none"
                    :disabled="
                      isAiRewriting || !aiPrompt.trim() || !selectedLlmModelId
                    "
              ...
              <div class="flex-1 flex flex-col min-w-0">
                <!-- 输出头部 -->
                <div
                  v-if="aiResult && !isAiRewriting"
                  class="shrink-0 px-5 py-2 border-b border-border-subtle flex items-center justify-between"
                >
                  <span class="text-[11px] text-text-secondary">
                    {{ isEditingAiResult ? "编辑模式" : "输出结果" }}
                  </span>
                  <div class="flex items-center gap-2">
                    <button
                      v-if="!isEditingAiResult"
                      class="px-2 py-1 text-[10px] border border-border-subtle rounded hover:bg-elevated text-text-secondary flex items-center gap-1 transition-colors"
                      @click="startEditAiResult"
                    >
                      <svg
                        class="w-3 h-3"
                        fill="none"
                    ...
                    <template v-else>
                      <button
                        class="px-2 py-1 text-[10px] border border-red-500/30 rounded hover:bg-red-500/10 text-red-400 transition-colors"
                        @click="cancelEditAiResult"
                      >
                        取消
                      </button>
                      <button
                        class="px-2 py-1 text-[10px] bg-emerald-600 text-white rounded hover:bg-emerald-500 font-medium transition-colors"
                        @click="saveEditAiResult"
                      >
                        保存
                      </button>
                    </template>
                ...
                <div class="flex-1 overflow-y-auto p-5">
                  ...
                  <div
                    v-else-if="isAiRewriting && !aiResult"
                  ...
                  >
                    <div class="flex items-center gap-2 text-purple-400">
                      ...
                      <span class="text-sm">AI 改写中...</span>
                    </div>
                  </div>
                  <div v-else-if="isEditingAiResult" class="w-full h-full">
                    <JsonEditor
                      v-if="viewMode === 'json'"
                      :model-value="editAiResultText"
                      @update:model-value="editAiResultText = $event"
                    />
                    <textarea
                      v-else
                      v-model="editAiResultText"
                      class="w-full h-full bg-input-bg border border-border-subtle rounded-lg px-4 py-3 text-sm text-text-primary focus:outline-none focus:border-purple-500/30 resize-none font-sans leading-relaxed"
                    ></textarea>
                  </div>
                  <pre
                    v-else
                    class="text-sm text-text-primary whitespace-pre-wrap break-words font-sans leading-relaxed">{{ aiResult }}<span v-if="isAiRewriting" class="inline-block w-[2px] h-[14px] bg-purple-400 ml-[1px] animate-pulse align-middle"></span></pre>
                </div>

                <!-- 确认按钮 -->
                <div
                  v-if="aiResult && !isAiRewriting"
                  class="shrink-0 px-5 py-3 border-t border-border-subtle"
                >
                  <button
                    class="w-full px-4 py-2.5 bg-gradient-to-r from-emerald-500 to-cyan-600 text-white rounded-lg hover:opacity-90 text-sm font-medium flex items-center justify-center gap-2 transition-opacity shadow-lg shadow-emerald-500/20"
                    @click="applyAiResult"
                  >
                    <svg
                      class="w-4 h-4"
...
const aiResult = ref("");
const isEditingAiResult = ref(false);
const editAiResultText = ref("");
...
const closeAiRewriteModal = () => {
  ...
  aiResult.value = "";
  isEditingAiResult.value = false;
  editAiResultText.value = "";
};

/** AI 改写内容（流式输出） */
const handleAiRewrite = async () => {
  if (!aiPrompt.value.trim()) {
    toast.warning("请输入 AI 改写指令");
    return;
  }

  const selectedModel = props.llmModels?.find(
    (m) => m.id === selectedLlmModelId.value,
  );
  if (!selectedModel) {
    toast.warning("请选择一个 LLM 模型");
    return;
  }
  ...
  try {
    let existingContent = "";
    if (includeExistingContent.value) {
      if (viewMode.value === "prompt") {
        existingContent = promptText.value || "";
      } else {
        // JSON 模式：获取当前页面的 JSON 内容
        if (props.page) {
          const filtered: Record<string, unknown> = {};
          for (const [k, v] of Object.entries(props.page)) {
            if (!k.startsWith("_")) filtered[k] = v;
          }
          existingContent = JSON.stringify(filtered, null, 2);
        }
      }
    }
    ...
    const systemPrompt =
      viewMode.value === "prompt"
        ? "你是一个专业的漫画提示词改写助手。用户会提供原始提示词和改写要求，请根据要求改写提示词。只返回改写后的提示词内容，不要添加任何解释或额外文字。"
        : "你是一个专业的漫画 JSON 数据改写助手。用户会提供原始 JSON 数据和改写要求，请根据要求改写 JSON。只返回改写后的有效 JSON，不要添加任何解释、注释或 markdown 标记。";
    ...
    if (!result.success) {
      toast.error(result.error || "AI 改写失败");
    }
  } catch (error) {
    toast.error(
      "AI 改写失败：" + (error instanceof Error ? error.message : "未知错误"),
    );
  } finally {
...
/** 开始编辑 AI 结果 */
const startEditAiResult = () => {
  editAiResultText.value = aiResult.value;
  isEditingAiResult.value = true;
};

/** 取消编辑 AI 结果 */
const cancelEditAiResult = () => {
  isEditingAiResult.value = false;
  editAiResultText.value = "";
};

/** 保存编辑 AI 结果 */
const saveEditAiResult = () => {
  aiResult.value = editAiResultText.value;
  isEditingAiResult.value = false;
  editAiResultText.value = "";
  toast.success("已保存编辑");
};
...
/** 应用 AI 改写结果 */
const applyAiResult = () => {
  const finalResult = isEditingAiResult.value
    ? editAiResultText.value
    : aiResult.value;
  if (!finalResult) return;

  if (viewMode.value === "prompt") {
    promptText.value = finalResult.trim();
    ...
    try {
      ...
      toast.error("AI 返回的内容不是有效的 JSON 格式");
      return;
    }
  }
