<template>
  <section
    class="w-full h-full p-6 md:p-10 flex flex-col bg-slate-50 overflow-y-auto"
  >
    <div class="max-w-4xl mx-auto w-full">
      ...
      >
        ...
        <div class="p-5 space-y-4">
          ...
          <div
            v-else
            class="bg-amber-50 border border-amber-100 p-4 rounded-xl"
          >
            <div class="flex items-center gap-3">
              <div
      ...
      >
        <div
          class="bg-slate-800 px-4 py-2 flex items-center gap-2 border-b border-slate-700"
        >
          ...
          <span
            class="text-[10px] text-slate-400 font-mono ml-2 tracking-widest"
            >DEPLOY_TERMINAL</span
          >
        </div>
        <div
          ref="consoleRef"
      ...
      >
        <div class="flex items-center gap-5">
          <div
            class="w-14 h-14 bg-green-50 text-green-600 border border-green-100 rounded-full flex items-center justify-center text-3xl shadow-sm"
          >
            <svg
              class="w-6 h-6"
              fill="none"
          ...
          <div>
            ...
            <p class="text-sm text-slate-500">
              所有批次内容已推送至绑定的微信公众号，可前往微信后台确认群发。
            </p>
...
function buildArticleContentHtml(
  ...
  const templateId =
    !layoutConfig.inheritGlobal && layoutConfig.templateId
      ? layoutConfig.templateId
      : globalConfig.layout.templateId;

  if (templateId === "flow" || templateId === "card" || !templateId) {
    return buildBuiltinTemplateHtml(templateId || "flow", article.images);
  }

  const customTemplate = templateStore.customTemplates.find(
    (t) => t.id === templateId,
  );
  if (!customTemplate) {
    return buildBuiltinTemplateHtml("flow", article.images);
  }
...
const syncArticles = computed<SyncArticleItem[]>(() => {
  ...
  return buildArticleParams(batchStore.articles, batchStore.globalConfig, {
    buildContentHtml: (article, globalConfig) =>
      buildArticleContentHtml(article, globalConfig),
  });
});
