...
electron.contextBridge.exposeInMainWorld("electronAPI", {
  ...
  comic: {
    ...
    llm: {
      fetchStart: (request) => electron.ipcRenderer.invoke("comic:llmFetchStart", request),
      fetchAbort: (requestId) => electron.ipcRenderer.send("comic:llmFetchAbort", requestId),
      onFetchEvent: (callback) => {
        const listener = (_event, payload) => callback(payload);
        electron.ipcRenderer.on("comic:llmFetchEvent", listener);
        return () => electron.ipcRenderer.off("comic:llmFetchEvent", listener);
      }
    },
